...
class BotReplyController extends BaseController
{
    ...
    public function processBotReplyCreate(BaseRequest $request)
    {
        ...
        if($request->message_type == 'question') {
            $validations['question_variable_name'] = [
                'required',
                'regex:/^[a-zA-Z_][a-zA-Z0-9_]*$/',
                'max:50'
            ];
            $validations['question_input_type'] = [
                'nullable',
                Rule::in(['', 'text', 'number', 'email', 'phone'])
            ];
            $validations['question_store_field'] = 'nullable|string|max:50';
            $validations['question_is_required'] = 'nullable|boolean';
            $validations['question_default_next_node'] = 'nullable|string';
            $validations['conditional_flows'] = 'nullable|array|max:10';

            if (!empty($request->conditional_flows)) {
                foreach ($request->conditional_flows as $index => $flow) {
                    $validations["conditional_flows.$index.label"] = 'nullable|string|max:100';
                    $validations["conditional_flows.$index.condition_type"] = [
                        'required',
                        Rule::in(['equals', 'contains', 'starts_with', 'regex', 'number_range'])
                    ];
                    $validations["conditional_flows.$index.condition_value"] = 'required|string|max:200';
                    $validations["conditional_flows.$index.target_node"] = 'required|string';
                }
            }
        }
        if(in_array($request->message_type, [
